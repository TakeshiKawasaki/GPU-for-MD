...
__global__ void eom_kernel_CP(double*x_dev,double*y_dev,double *vx_dev,double *vy_dev,double *fx_dev,double *fy_dev,double *L_dev,double *dt_dev, int *FIRE_gate_dev,double *gamma_dev,double *pressure_dev,double *vol_dev,double *vol_dot_dev){
  int i_global = threadIdx.x + blockIdx.x*blockDim.x;
  double y_temp;
  double L_dev_temp;
  ...
  if(i_global<NP){
    ...
    L_dev_temp=*L_dev;
    *vol_dev = *L_dev**L_dev;
    *vol_dot_dev += NP*(pressure_dev[0] - P0)*(dt_dev[0]);
    *vol_dev += (*vol_dot_dev)*(dt_dev[0]);
    *L_dev = sqrt(*vol_dev);
...
__global__ void FIRE_synth_dev_CP(double *vx_dev,double *vy_dev, double *fx_dev, double *fy_dev, double *power_dev,double *alpha_dev,int *FIRE_gate_dev,double *pressure_dev,double *vol_dot_dev,double *L_dev){
  int i_global = threadIdx.x + blockIdx.x*blockDim.x;
  double f,v,deltaP;
  if(i_global<NP){
    ...
    deltaP = pressure_dev[0]-P0;
    *vol_dot_dev =  (1.-alpha_dev[0])**vol_dot_dev + alpha_dev[0]*deltaP/(sqrt(deltaP*deltaP)+DBL_EPSILON)*sqrt(*vol_dot_dev**vol_dot_dev);
...
__global__ void FIRE_reset_dev_CP(double *vx_dev, double *vy_dev,double *power_dev,double *alpha_dev,double *dt_dev,int *FIRE_param_gate_dev,double *vol_dot_dev, double *pressure_dev, double *L_dev){
  int i_global = threadIdx.x + blockIdx.x*blockDim.x;
...
__global__ void FIRE_reset_dev_SS(double *vx_dev, double *vy_dev,double *power_dev,double *alpha_dev,double *dt_dev,int *FIRE_param_gate_dev,double *gamma_dot_dev, double *stress_dev, double *L_dev){
  ...
  if(i_global<NP){
    if(power_dev[0] - stress_dev[0]**gamma_dot_dev*(*L_dev)*(*L_dev) < 0){
      vx_dev[i_global] = 0.0; vy_dev[i_global] = 0.0;
      if(i_global == 0){
	*gamma_dot_dev=0.0;
	alpha_dev[0] = 0.1;
...
__global__ void calc_force_kernel(double*x_dev,double*y_dev,double *fx_dev,double *fy_dev,double *a_dev,double *L_dev,int *list_dev,double *gamma_dev, double *stress_dev,double *pressure_dev){
...
__global__ void change_deltaphi(double *deltaphi_dev,double *phi_dev, double *pot_dev){
  if(*phi_dev >= phimax-1.e-6)
    *deltaphi_dev = -1.e-4;
  if(*deltaphi_dev < 0 && pot_dev[0] < 1.e-9)
    *deltaphi_dev = -1.e-5;
  if(*deltaphi_dev < 0 && pot_dev[0] < 1.e-11)
    *deltaphi_dev = -1.e-6;
   __syncthreads();
}
...
void FIRE(double *x_dev, double *y_dev,double  *vx_dev, double *vy_dev, double *fx_dev,double *fy_dev, double *a_dev,double *L_dev, int *list_dev, double *power_dev, double *alpha_dev, double *dt_dev, int *FIRE_gate_dev,int *FIRE_param_gate_dev, int *reduce_dev, int *remain_dev,double *gamma_dev,double *stress_dev,double *pressure_dev){

  calc_force_kernel<<<NB,NT>>>(x_dev,y_dev,fx_dev,fy_dev,a_dev,L_dev,list_dev,gamma_dev,stress_dev,pressure_dev);
...
void FIRE_CP(double *x_dev, double *y_dev,double  *vx_dev, double *vy_dev, double *fx_dev,double *fy_dev, double *a_dev,double *L_dev, int *list_dev, double *power_dev, double *alpha_dev, double *dt_dev, int *FIRE_gate_dev,int *FIRE_param_gate_dev, int *reduce_dev, int *remain_dev,double *gamma_dev, double *stress_dev,double *pressure_dev,double *vol_dev, double *vol_dot_dev){

  init_scalar_kernel<<<1,1>>>(vol_dot_dev,0.0);
  calc_force_kernel<<<NB,NT>>>(x_dev,y_dev,fx_dev,fy_dev,a_dev,L_dev,list_dev,gamma_dev,stress_dev,pressure_dev);
  ...
  eom_kernel_CP<<<NB,NT>>>(x_dev,y_dev,vx_dev,vy_dev,fx_dev,fy_dev,L_dev,dt_dev,FIRE_gate_dev,gamma_dev,pressure_dev,vol_dev,vol_dot_dev);
  FIRE_synth_dev_CP<<<NB,NT>>>(vx_dev,vy_dev,fx_dev,fy_dev,power_dev,alpha_dev,FIRE_gate_dev,pressure_dev,vol_dot_dev,L_dev);
  double sum_power = thrust::reduce(thrust::device_pointer_cast(power_dev), thrust::device_pointer_cast(power_dev + NB * NT),0.0,thrust::plus<double>());
  cudaMemcpy(&power_dev[0], &sum_power, sizeof(double), cudaMemcpyHostToDevice);
  FIRE_reset_dev_CP<<<NB,NT>>>(vx_dev,vy_dev,power_dev,alpha_dev,dt_dev,FIRE_param_gate_dev,vol_dot_dev,pressure_dev,L_dev);
}
...
int main(int argc, char** argv){
  ...
  double gamma =0.,*gamma_dev,*gamma_observe_dev,*delta_gamma_dev,*gamma_dot_dev,*vol_dev,*vol_dot_dev;
  ...
  cudaMalloc((void**)&vol_dev,sizeof(double));
  cudaMalloc((void**)&vol_dot_dev,sizeof(double));
  ...
  sprintf(filename,"stress_phimax%f_P%f_ens%d.dat",phimax,P0,gpu_id);
  ofstream file;
  file.open(filename);
  ...
  for(;;){
    FIRE(x_dev,y_dev,vx_dev,vy_dev,fx_dev,fy_dev,a_dev,L_dev,list_dev,power_dev,alpha_dev,dt_dev,FIRE_gate_dev,FIRE_param_gate_dev,reduce_dev,remain_dev,gamma_dev,stress_dev,pressure_dev);                                         
  ...
  double sum_pressure,sum_pot;
    for(;;){
      measureTime();  
      for(;;){
	clock++;
	FIRE_SS(x_dev,y_dev,vx_dev,vy_dev,fx_dev,fy_dev,a_dev,L_dev,list_dev,power_dev,alpha_dev,dt_dev,FIRE_gate_dev,FIRE_param_gate_dev,reduce_dev,remain_dev,gamma_dev,gamma_dot_dev,stress_dev,pressure_dev);
	list_auto_update(L_dev,x_dev,y_dev,vx_dev,vy_dev,dx_dev,dy_dev,dt_dev,M,map_dev,gate_dev,list_dev,gamma_dev);
	cudaMemcpy(&FIRE_gate,FIRE_gate_dev,sizeof(int),cudaMemcpyDeviceToHost);
	if(FIRE_gate == 1){
	  calc_energy_kernel<<<NB,NT>>>(x_dev,y_dev,pressure_dev,stress_dev,pot_dev,a_dev,L_dev,list_dev,gamma_dev);
	  // sum_pressure = thrust::reduce(thrust::device_pointer_cast(pressure_dev), thrust::device_pointer_cast(pressure_dev + NB * NT),0.0,thrust::plus<double>());
	  // sum_pot = thrust::reduce(thrust::device_pointer_cast(pot_dev), thrust::device_pointer_cast(pot_dev + NB * NT),0.0,thrust::plus<double>());
	  // cudaMemcpy(&pot_dev[0],&sum_pot,NB*NT*sizeof(double),cudaMemcpyHostToDevice);
	  ...
	  cout<<"phi= "<< phi << " count= "<< clock<<" energy="<< pot[0] << " pressure= "<< pressure[0]  <<" stress= "<< stress[0] <<" gamma= "<< gamma   <<endl;
	  clock = 0;
	  break;
	}
	//////////////////////////
      }
      if(pressure[0] < P0 && deltaphi<0){
	cout<<" phiJ= "<< phi <<endl;
	break;
      }
    ...
    ///initiallization/////////////////
    init_scalar_kernel<<<1,1>>>(delta_gamma_dev, 1.e-8);    
    ...
    while(gamma < 0.5){ 
      ...
      for(;;){
	clock++;
	FIRE_CP(x_dev,y_dev,vx_dev,vy_dev,fx_dev,fy_dev,a_dev,L_dev,list_dev,power_dev,alpha_dev,dt_dev,FIRE_gate_dev,FIRE_param_gate_dev,reduce_dev,remain_dev,gamma_dev,stress_dev,pressure_dev,vol_dev,vol_dot_dev);
	list_auto_update(L_dev,x_dev,y_dev,vx_dev,vy_dev,dx_dev,dy_dev,dt_dev,M,map_dev,gate_dev,list_dev,gamma_dev);   
	cudaMemcpy(&FIRE_gate,FIRE_gate_dev,sizeof(int),cudaMemcpyDeviceToHost);
	if(FIRE_gate == 1){
	  calc_energy_kernel<<<NB,NT>>>(x_dev,y_dev,pressure_dev,stress_dev,pot_dev,a_dev,L_dev,list_dev,gamma_dev);
	  ...
	  phi = M_PI*(1.0*1.0+1.4*1.4)*(double)NP/(8.*L*L);
	  cout<<"gamma= "<< gamma << " count= "<< clock <<" stress= "<<stress[0] <<" pressure= "<< pressure[0]<<" phi"<<phi <<endl;
	  file<< gamma<<" "<<stress[0]<<" "<< pressure[0] <<" "<< phi <<endl;
	  clock = 0;
	  break;
	}
      }
